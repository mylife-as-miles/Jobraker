  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        ...
        {/* Time Range Selector */}
        <div className="flex gap-2 bg-gradient-to-br from-background via-[#111111] to-background border border-[#2dd4bf]/20 rounded-xl p-1">
          {[30, 60, 90].map((days) => (
            <button
              key={days}
              onClick={() => setTimeRange(days as any)}
              className={`px-4 py-2 rounded-lg font-medium text-sm transition-all ${timeRange === days
                ? 'bg-[#2dd4bf] text-black shadow-lg shadow-[#2dd4bf]/50'
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        ...
        >
          <Card className="bg-gradient-to-br from-[#22d3ee]/10 to-background/10 border-[#2dd4bf]/30 hover:border-[#2dd4bf]/60 hover:shadow-lg hover:shadow-[#2dd4bf]/20 transition-all duration-300">
            <CardContent className="p-6">
              <div className="flex items-center justify-between mb-4">
                <div className="w-12 h-12 rounded-xl bg-gradient-to-br from-[#22d3ee]/20 to-background/20 border border-[#2dd4bf]/30 flex items-center justify-center">
      ...
      >
        <Card className="bg-gradient-to-br from-background via-[#111111] to-background border-[#2dd4bf]/20 hover:border-[#2dd4bf]/50 hover:shadow-lg hover:shadow-[#2dd4bf]/20 transition-all duration-300">
          <CardHeader>
            ...
            <CardContent>
              <ResponsiveContainer width="100%" height={300}>
                <BarChart data={chartData}>
                  ...
                  />
                  <Bar dataKey="newSubs" fill="#2dd4bf" radius={[8, 8, 0, 0]} name="New Subscriptions" />
                </BarChart>
              </ResponsiveContainer>
            </CardContent>
          </Card>
            ...
            <CardContent>
              <div className="space-y-4">
                ...
                {(() => {
                  ...
                  return (
                    <div className="flex items-center justify-between p-4 bg-[#2dd4bf]/10 border border-[#2dd4bf]/20 rounded-xl">
                      <div className="flex items-center gap-3">
                      ...
                      <div className="text-right">
                        ...
                        <p className="text-sm text-[#2dd4bf]">{proData.percentage}% of MRR</p>
                      </div>
                    </div>
                  );
                })()}
        ...
        <Card className="bg-gradient-to-br from-background via-[#111111] to-background border-[#2dd4bf]/20">
          <CardHeader className="flex flex-row items-center justify-between">
            <div>
              ...
              <p className="text-sm text-gray-400">Real-time financial activity</p>
            </div>
          </CardHeader>
          <CardContent>
            {/* Filters */}
            <div className="flex flex-col md:flex-row gap-4 mb-6">
              <div className="flex-1 relative">
                ...
                <input
                  ...
                  className="w-full pl-10 pr-4 py-2 bg-gray-900/50 border border-gray-700 rounded-lg text-white placeholder-gray-500 focus:outline-none focus:border-[#2dd4bf]/50"
                />
              </div>
              <div className="flex gap-2">
                <select
                  ...
                  className="px-4 py-2 bg-gray-900/50 border border-gray-700 rounded-lg text-white focus:outline-none focus:border-[#2dd4bf]/50"
                >
              ...
              <table className="w-full">
                <thead>
                  <tr className="border-b border-gray-800">
                    <th className="text-left py-3 px-4 text-xs font-medium text-gray-400 uppercase tracking-wider">Time</th>
                ...
                <tbody className="divide-y divide-gray-800">
                  {filteredTransactions.map((tx) => (
                    ...
                    >
                      <td className="py-3 px-4">
                        <div className="flex items-center gap-2 text-gray-400">
                          ...
                          <span className="text-sm">
                            ...
                            <span className="text-gray-600 ml-1">{new Date(tx.created_at).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}</span>
                          </span>
                        </div>
                      </td>
                      <td className="py-3 px-4">
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            <p className="text-sm font-medium text-white group-hover:text-[#2dd4bf] transition-colors">
                      ...
                      <td className="py-3 px-4">
                        <span className={`inline-flex items-center px-2 py-0.5 rounded text-xs font-medium border ${tx.transaction_type === 'purchase' ? 'bg-[#2dd4bf]/10 text-[#2dd4bf] border-[#2dd4bf]/20' :
                          ...
                          {tx.transaction_type === 'deduction' && <ArrowDownRight className="w-3 h-3 mr-1" />}
                      ...
                      <td className="py-3 px-4">
                        ...
                        {tx.reference_type && (
                          <span className="text-xs text-gray-600 uppercase tracking-wider">{tx.reference_type}</span>
                        )}
                      </td>
                      <td className="py-3 px-4 text-right">
                        <span className={`font-mono font-medium ${tx.amount > 0 ? 'text-[#2dd4bf]' : 'text-gray-400'
                          }`}>
        ...
        {isDetailOpen && selectedTransaction && (
          <>
            ...
            <motion.div
              ...
              className="fixed right-0 top-0 bottom-0 w-full max-w-md bg-background border-l border-[#2dd4bf]/20 z-50 overflow-y-auto"
            >
              <div className="p-6">
                ...
                <div className="space-y-8">
                  {/* Amount Badge */}
                  <div className="text-center p-6 bg-gradient-to-br from-[#22d3ee]/10 to-transparent rounded-2xl border border-[#2dd4bf]/20">
                    <p className="text-sm text-gray-400 mb-1">Amount</p>
                    <p className="text-4xl font-bold text-[#2dd4bf]">
                  ...
                  {/* User Info */}
                  <div>
                    <h3 className="text-sm font-semibold text-gray-300 uppercase tracking-wider mb-4">User</h3>
                    <div className="flex items-center gap-4 p-4 bg-gray-900/50 rounded-xl border border-gray-800">
                      <div className="w-12 h-12 rounded-full bg-gray-800 flex items-center justify-center overflow-hidden border border-gray-700">
                        {selectedTransaction.user?.avatar_url ? (
                          <img src={selectedTransaction.user.avatar_url} alt="" className="w-full h-full object-cover" />
                        ) : (
                          <Users className="w-6 h-6 text-gray-400" />
                        )}
                      </div>
                      <div>
                        <p className="font-medium text-white">{selectedTransaction.user?.full_name || 'Unknown'}</p>
                  ...
                  {/* Transaction Details */}
                  <div>
                    <h3 className="text-sm font-semibold text-gray-300 uppercase tracking-wider mb-4">Details</h3>
                    <div className="space-y-4">
                      <div className="flex justify-between py-3 border-b border-gray-800">
                        <span className="text-gray-400">Transaction ID</span>
                        <span className="text-white font-mono text-xs">{selectedTransaction.id}</span>
                      </div>
                      <div className="flex justify-between py-3 border-b border-gray-800">
                        <span className="text-gray-400">Type</span>
                        <span className="text-white capitalize">{selectedTransaction.transaction_type}</span>
                      </div>
                      <div className="flex justify-between py-3 border-b border-gray-800">
                        <span className="text-gray-400">Category</span>
                        <span className="text-white capitalize">{selectedTransaction.reference_type || 'N/A'}</span>
                      </div>
                      <div className="flex justify-between py-3 border-b border-gray-800">
                      ...
                      <div className="flex justify-between py-3 border-b border-gray-800">
                        ...
                        <span className="text-white">{new Date(selectedTransaction.created_at).toLocaleTimeString()}</span>
                      </div>
                    </div>
                  </div>
                  ...
                  {/* Description Box */}
                  <div>
                    <h3 className="text-sm font-semibold text-gray-300 uppercase tracking-wider mb-4">Description</h3>
