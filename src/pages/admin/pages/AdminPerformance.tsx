...
import { Activity, Server, Database, HardDrive, CheckCircle2, AlertCircle, Loader2, ArrowUpRight, Clock, Shield, X, Code } from 'lucide-react';
        ...
        >
          ...
          <div className="p-6 border-b border-[#2dd4bf]/10 flex items-center justify-between sticky top-0 bg-background/95 backdrop-blur z-10">
            <div>
              <h2 className="text-xl font-bold text-white flex items-center gap-2">
                {service.type === 'database' && <Database className="w-5 h-5 text-[#2dd4bf]" />}
                {service.type === 'auth' && <Shield className="w-5 h-5 text-[#2dd4bf]" />}
                {service.type === 'storage' && <HardDrive className="w-5 h-5 text-[#2dd4bf]" />}
          ...
          <div className="p-6 space-y-8">
            ...
            <div className={`flex flex-col items-center justify-center p-8 rounded-2xl border ${service.status === 'healthy'
              ...
              <h3 className={`text-2xl font-bold capitalize ${service.status === 'healthy' ? 'text-[#2dd4bf]' : 'text-red-500'}`}>
                {service.status}
              </h3>
            ...
            <div className="space-y-4">
              <h3 className="text-sm font-semibold text-gray-500 uppercase tracking-wider">Check Details</h3>
              <div className="space-y-3">
                <div className="p-4 bg-gray-800/30 rounded-xl border border-gray-800 flex justify-between items-center">
                  <span className="text-gray-400 text-sm">Timestamp</span>
                  <span className="text-white font-mono text-sm">{service.lastChecked.toLocaleTimeString()}</span>
                </div>
                <div className="p-4 bg-gray-800/30 rounded-xl border border-gray-800 flex justify-between items-center">
                  <span className="text-gray-400 text-sm">Response Message</span>
                  <span className="text-white text-sm">{service.message || 'OK'}</span>
                </div>
              </div>
            </div>
...
export default function AdminPerformance() {
  ...
  const [services, setServices] = useState<ServiceStatus[]>([
    { name: 'Primary Database', type: 'database', status: 'checking', latency: 0, lastChecked: new Date() },
    { name: 'Authentication', type: 'auth', status: 'checking', latency: 0, lastChecked: new Date() },
    { name: 'Storage Bucket', type: 'storage', status: 'checking', latency: 0, lastChecked: new Date() },
  ]);
  ...
  const measureLatency = async (fn: () => Promise<any>): Promise<{ latency: number, result: any, error?: any }> => {
  ...
  const runChecks = async () => {
    ...
    const newServices: ServiceStatus[] = [
      ...
      },
      {
        ...
        message: authCheck.error ? authCheck.error.message : 'Session services active',
        lastChecked: now,
      ...
      },
      {
        ...
        message: storageCheck.error ? storageCheck.error.message : 'Buckets accessible',
        lastChecked: now,
  ...
  const avgLatency = useMemo(() => {
    ...
    return Math.round(services.reduce((sum, s) => sum + s.latency, 0) / services.length);
  }, [services]);

  const healthyServices = services.filter(s => s.status === 'healthy').length;
  const systemStatus = healthyServices === services.length ? 'Healthy' : 'Degraded';
  ...
  return (
    <div className="space-y-6">
      ...
      <div>
        ...
        <p className="text-gray-400">Real-time performance monitoring and health checks</p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
        ...
        >
          <div className="flex items-center gap-4">
            <div className={`w-12 h-12 rounded-xl flex items-center justify-center ${systemStatus === 'Healthy' ? 'bg-[#2dd4bf]/20 text-[#2dd4bf]' : 'bg-red-500/20 text-red-500'
              }`}>
              <Activity className="w-6 h-6" />
            </div>
            <div>
              <p className="text-sm text-gray-400">System Status</p>
              <p className={`text-2xl font-bold ${systemStatus === 'Healthy' ? 'text-[#2dd4bf]' : 'text-red-500'
                }`}>{systemStatus}</p>
            </div>
          </div>
        </motion.div>
        ...
        >
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-2xl font-bold text-white">{loading ? '...' : avgLatency + 'ms'}</p>
            </div>
          </div>
        </motion.div>
        ...
        >
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-2xl font-bold text-white">{healthyServices}/{services.length}</p>
            </div>
          </div>
        </motion.div>
      ...
      >
        <div className="mb-6 flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            <span className="flex items-center gap-1 text-xs text-[#2dd4bf]"><div className="w-2 h-2 rounded-full bg-[#2dd4bf]" /> Database</span>
      ...
      >
        <div className="p-6 border-b border-[#2dd4bf]/20">
        ...
        <div className="overflow-x-auto">
          <table className="w-full">
            <thead className="bg-gray-900/50 border-b border-gray-800">
              <tr>
                <th className="px-6 py-4 text-left text-xs font-semibold text-gray-400 uppercase tracking-wider">Service Name</th>
                <th className="px-6 py-4 text-left text-xs font-semibold text-gray-400 uppercase tracking-wider">Status</th>
                <th className="px-6 py-4 text-right text-xs font-semibold text-gray-400 uppercase tracking-wider">Latency</th>
                <th className="px-6 py-4 text-left text-xs font-semibold text-gray-400 uppercase tracking-wider">Message</th>
                <th className="px-6 py-4 text-right text-xs font-semibold text-gray-400 uppercase tracking-wider">Last Check</th>
                <th className="px-6 py-4 text-right text-xs font-semibold text-gray-400 uppercase tracking-wider">Action</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-gray-800">
              {loading ? (
                <tr><td colSpan={6} className="px-6 py-8 text-center text-gray-500">Initializing checks...</td></tr>
              ) : (
                  ...
                  >
                    <td className="px-6 py-4">
                      <div className="flex items-center gap-3">
                        <div className={`w-8 h-8 rounded-lg flex items-center justify-center ${service.type === 'database' ? 'bg-[#2dd4bf]/20' :
                          service.type === 'auth' ? 'bg-blue-500/20' : 'bg-purple-500/20'
                          }`}>
                          {service.type === 'database' && <Database className={`w-4 h-4 ${service.type === 'database' ? 'text-[#2dd4bf]' : ''}`} />}
                    ...
                    <td className="px-6 py-4">
                      <span className={`inline-flex items-center gap-1.5 px-2.5 py-0.5 rounded-full text-xs font-medium ${service.status === 'healthy'
                        ...
                        {service.status === 'checking' && <Loader2 className="w-3 h-3 animate-spin" />}
