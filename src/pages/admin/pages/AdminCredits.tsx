...
import { Coins, TrendingUp, TrendingDown, Loader2, Filter, ChevronDown, X, FileText, ArrowUpRight, ArrowDownRight, Search, Calendar, User as UserIcon, Users } from 'lucide-react';
        ...
        >
          ...
          <div className="p-6 space-y-8">
            ...
            <div className={`flex flex-col items-center justify-center p-8 rounded-2xl border ${isPositive
              ...
              <h3 className={`text-4xl font-bold ${isPositive ? 'text-[#1dff00]' : 'text-red-500'}`}>
            ...
            <div className="space-y-4">
              ...
              <div className="flex items-center gap-4 p-4 bg-gray-800/30 rounded-xl border border-gray-800">
                ...
                <div>
                  <p className="text-white font-medium">{transaction.user.full_name || 'Unknown User'}</p>
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                {transaction.balance_before !== undefined && (
                  <div className="p-4 bg-gray-800/30 rounded-xl border border-gray-800">
                    ...
                    <p className="text-white font-mono">{transaction.balance_before}</p>
                  </div>
                )}
                {transaction.balance_after !== undefined && (
                  <div className="p-4 bg-gray-800/30 rounded-xl border border-gray-800">
                    <p className="text-gray-400 text-xs mb-1">Balance After</p>
                    <p className="text-white font-mono">{transaction.balance_after}</p>
                  </div>
                )}
              </div>
            </div>
...
export default function AdminCredits() {
  ...
  // Data hooks
  const { transactions: recentTransactions, loading: txLoading } = useRecentTransactions(200);
  ...
  const filteredTransactions = useMemo(() => {
    return recentTransactions
      .filter(tx => {
        const matchesType = filterType === 'all' || (tx.transaction_type || tx.type) === filterType;
        const matchesSearch =
      ...
      .sort((a, b) => {
        const aVal = sortField === 'amount' ? Math.abs(a.amount) : new Date(a.created_at).getTime();
        const bVal = sortField === 'amount' ? Math.abs(b.amount) : new Date(b.created_at).getTime();
  ...
  const getTypeColor = (type: string | undefined) => {
    switch (type) {
      ...
      case 'consumed':
      case 'spent':
      case 'deduction': return 'text-red-400 bg-red-500/20 border-red-500/30';
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        <motion.div
          ...
          className="bg-[#0a0a0a] border border-[#1dff00]/20 rounded-2xl p-6 relative overflow-hidden group"
        >
          ...
          <div className="relative z-10">
            <div className="flex items-center justify-between mb-4">
              ...
              <span className="text-xs font-medium text-[#1dff00] bg-[#1dff00]/10 px-2 py-1 rounded-full">Available</span>
            </div>
        ...
        <motion.div
          ...
          className="bg-[#0a0a0a] border border-blue-500/20 rounded-2xl p-6 relative overflow-hidden group"
        >
          ...
          <div className="relative z-10">
            <div className="flex items-center justify-between mb-4">
              ...
              <span className="text-xs font-medium text-blue-400 bg-blue-500/10 px-2 py-1 rounded-full">Issued</span>
            </div>
        ...
        <motion.div
          ...
          className="bg-[#0a0a0a] border border-red-500/20 rounded-2xl p-6 relative overflow-hidden group"
        >
          ...
          <div className="relative z-10">
            <div className="flex items-center justify-between mb-4">
              ...
              <span className="text-xs font-medium text-red-400 bg-red-500/10 px-2 py-1 rounded-full">Consumed</span>
            </div>
            <p className="text-3xl font-bold text-white mb-1">{stats.totalConsumed.toLocaleString()}</p>
        ...
        <motion.div
          ...
          className="bg-[#0a0a0a] border border-purple-500/20 rounded-2xl p-6 relative overflow-hidden group"
        >
          ...
          <div className="relative z-10">
            <div className="flex items-center justify-between mb-4">
              ...
              <span className="text-xs font-medium text-purple-400 bg-purple-500/10 px-2 py-1 rounded-full">Average</span>
            </div>
            <p className="text-3xl font-bold text-white mb-1">{stats.avgPerUser.toFixed(0)}</p>
            ...
            <tbody className="divide-y divide-gray-800">
              {filteredTransactions.length === 0 ? (
                ...
                filteredTransactions.slice(0, 50).map((tx, index) => {
                  const txType = tx.transaction_type || tx.type;
                  const isNegative = txType === 'consumed' || txType === 'spent' || txType === 'deduction';
                    ...
                    >
                      ...
                      <td className="px-6 py-4">
                        <div className="flex items-center gap-3">
                          {tx.user?.avatar_url ? (
                            <img src={tx.user.avatar_url} alt="" className="w-8 h-8 rounded-full object-cover border border-gray-700" />
                          ) : (
                          ...
                          <div className="truncate max-w-[150px]">
                            <p className="text-sm font-medium text-white truncate">{tx.user?.full_name || 'Unknown User'}</p>
                            <p className="text-xs text-gray-500 truncate">{tx.user?.email}</p>
                          </div>
                        </div>
                      </td>
                      <td className="px-6 py-4">
                        <span className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-lg border text-xs font-medium ${getTypeColor(txType)}`}>
                          {txType}
                        </span>
                      </td>
                      ...
                      <td className={`px-6 py-4 text-right font-bold font-mono ${isNegative ? 'text-red-400' : 'text-[#1dff00]'
                        }`}>
