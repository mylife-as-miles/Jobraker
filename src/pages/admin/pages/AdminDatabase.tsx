    ...
    <AnimatePresence>
      ...
      >
        <motion.div
          ...
          className="w-full max-w-xl h-full bg-[#0a0a0a] border-l border-[#1dff00]/20 shadow-2xl overflow-y-auto"
                ...
                <div className="space-y-4">
                  ...
                  <div className="relative group">
                    ...
                    <div className="absolute top-2 right-2 opacity-0 group-hover:opacity-100 transition-opacity">
                      <span className="px-2 py-1 bg-gray-800 rounded text-[10px] text-gray-400">Read-only view</span>
                    </div>
                  </div>
                </div>
              </>
    ...
    await Promise.all(CORE_TABLES.map(async (table) => {
      try {
        ...
        if (error) {
          ...
          stats.push({ name: table, rows: null, size: 'Unknown', updated: 'Unknown', loading: false, error: error.message });
        } else {
          stats.push({ name: table, rows: count, size: 'Unknown', updated: 'Unknown', loading: false });
        }
      } catch (err) {
        console.warn(`Error processing ${table}:`, err);
        stats.push({ name: table, rows: null, size: 'Unknown', updated: 'Unknown', loading: false, error: 'Failed to fetch' });
      }
    }));
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        >
          ...
          <div className="relative z-10">
            <div className="flex items-center justify-between mb-4">
              ...
              <span className="text-xs font-medium text-blue-400 bg-blue-500/10 px-2 py-1 rounded-full">Total Volume</span>
            </div>
            <p className="text-3xl font-bold text-white mb-1">{loading ? '...' : totalRows.toLocaleString()}</p>
        ...
        <motion.div
          ...
          className="bg-[#0a0a0a] border border-violet-500/20 rounded-2xl p-6 relative overflow-hidden group"
        >
          ...
          <div className="relative z-10">
            <div className="flex items-center justify-between mb-4">
              ...
              <span className="text-xs font-medium text-violet-400 bg-violet-500/10 px-2 py-1 rounded-full">System</span>
            </div>
            ...
            <thead className="bg-gray-900/50 border-b border-gray-800">
              <tr>
                ...
                <th
                  ...
                  onClick={() => { setSortField('rows'); setSortOrder(sortOrder === 'asc' ? 'desc' : 'asc'); }}
                >
                  Row Count
                </th>
              ...
              {loading ? (
                ...
                filteredTables.map((table, index) => (
                  <motion.tr
                    ...
                    onClick={() => { setSelectedTable(table.name); setIsDetailOpen(true); }}
                  >
                    ...
                    <td className="px-6 py-4 text-right">
                      {table.rows !== null ? (
                        <span className="text-white font-mono font-bold">{table.rows.toLocaleString()}</span>
                      ) : (
