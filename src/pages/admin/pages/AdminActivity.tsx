      ...
      >
        ...
        >
          ...
          <div className="p-6 space-y-8">
            {/* Action Badge */}
            <div className="flex flex-col items-center justify-center p-8 rounded-2xl border bg-[#2dd4bf]/5 border-[#2dd4bf]/20">
              <div className="w-16 h-16 rounded-full flex items-center justify-center mb-4 bg-[#2dd4bf]/10 text-[#2dd4bf]">
...
export default function AdminActivity() {
  ...
  const [filterType, setFilterType] = useState<'all' | 'job_search' | 'auto_apply'>('all');
  ...
  const relevantActivity = useMemo(() => {
    return recentTransactions.filter(t =>
      ...
      (t.transaction_type === 'deduction' || t.transaction_type === 'consumed' || t.transaction_type === 'spent') &&
      (t.reference_type === 'job_search' || t.reference_type === 'auto_apply')
    );
  }, [recentTransactions]);

  const stats = useMemo(() => {
    const jobSearches = relevantActivity.filter(t => t.reference_type === 'job_search');
    const autoApplies = relevantActivity.filter(t => t.reference_type === 'auto_apply');
  ...
  const filteredActivity = useMemo(() => {
    return relevantActivity
      .filter(tx => {
        const matchesType = filterType === 'all' || tx.reference_type === filterType;
        const matchesSearch =
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <motion.div
          ...
          className="bg-gradient-to-br from-[#22d3ee]/10 to-background/10 border border-[#2dd4bf]/30 rounded-2xl p-6"
        >
          ...
          <p className="text-3xl font-bold text-white">{stats.totalJobsFound.toLocaleString()}</p>
          <p className="text-xs text-gray-500 mt-1">{stats.jobSearches.length} search sessions</p>
        </motion.div>
        ...
        >
          ...
          <p className="text-3xl font-bold text-white">{stats.totalJobsApplied.toLocaleString()}</p>
          <p className="text-xs text-gray-500 mt-1">{stats.autoApplies.length} apply sessions</p>
        </motion.div>
      ...
      >
        <div className="mb-6">
          ...
          <p className="text-sm text-gray-400">Daily job searches and auto applies (recent)</p>
        </div>
              ...
              {filteredActivity.length === 0 ? (
                ...
                filteredActivity.slice(0, 50).map((activity, index) => (
                  <motion.tr
                    ...
                    onClick={() => { setSelectedTransaction(activity); setIsDetailOpen(true); }}
                  >
                    ...
                    <td className="px-6 py-4">
                      <div className="flex items-center gap-3">
                        ...
                        <div className="truncate max-w-[150px]">
                          <p className="text-sm font-medium text-white truncate">{activity.user?.full_name || 'Unknown User'}</p>
