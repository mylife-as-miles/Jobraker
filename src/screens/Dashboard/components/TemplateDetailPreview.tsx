...
export const TemplateDetailPreview = ({ templateId, onClose, onBack }: TemplateDetailPreviewProps) => {
    const storeMetadata = useArtboardStore((state: ArtboardStore) => state.resume.data.metadata);
    const setResumeData = useArtboardStore((state: ArtboardStore) => state.setResumeData);

    // Local state for live preview overrides
    const [primaryColor, setPrimaryColor] = useState(storeMetadata.theme?.primary || '#0df233');
    const [fontFamily, setFontFamily] = useState(storeMetadata.typography.font.family || 'Inter, sans-serif');
    ...
    const templateInfo = {
        azurill: { name: 'Azurill', description: 'A professional, structured template ideal for designers and developers. Features a timeline layout and clean typography.' },
        bronzor: { name: 'Bronzor', description: 'Structured grid layout, highly organized and space-efficient for technical profiles.' },
    ...
    }[templateId] || { name: 'Template', description: 'A professional resume template.' };
                ...
                <div className="p-6 flex flex-col gap-8 overflow-y-auto custom-scrollbar flex-1">
                    ...
                    <div>
                        <div className="flex items-center gap-2 mb-4">
                            <Palette className="w-4 h-4 text-[#2dd4bf]" />
                    ...
                    <div>
                        <div className="flex items-center gap-2 mb-4">
                            <TypeIcon className="w-4 h-4 text-[#2dd4bf]" />
                                ...
                                <button
                                    ...
                                    className={cn(
                                        ...
                                        fontFamily === font.family
                                            ? "bg-border border-[#2dd4bf] text-foreground shadow-[0_0_10px_rgba(45,212,191,0.1)]"
                    ...
                    <div className="mt-auto pt-6 border-t border-[#28392b] space-y-3">
                        <button
                            ...
                            className="w-full flex items-center justify-center gap-2 h-12 bg-[#2dd4bf] hover:bg-[#18d600] text-[#111812] rounded-lg font-bold text-base transition-all shadow-[0_0_20px_rgba(45,212,191,0.2)] active:scale-95"
                        >
            ...
            <div className="flex-1 flex flex-col relative overflow-hidden bg-background">
                ...
                <div className="h-14 border-b border-[#28392b] bg-foreground/50 backdrop-blur-sm flex items-center justify-between px-6 shrink-0">
                    <div className="flex items-center gap-4">
                        ...
                        <span className="text-sm font-bold text-[#2dd4bf] min-w-[3rem] text-center">
                            {Math.round(zoom * 100)}%
                        </span>
                        <button
                ...
                <div className="flex-1 overflow-auto preview-scroll flex justify-center items-start p-12 bg-grid-pattern">
                    ...
                    >
                        <TemplatePreview templateId={templateId} metadataOverride={metadataOverride} />
                    </div>
                </div>
            </div>
