        ...
        <div className="space-y-4 animate-in slide-in-from-top-2 duration-200">
            <div className="flex flex-wrap gap-2">
                {section.items.map((item) => (
                    <div
                        key={item.id}
                        className="group flex items-center gap-1.5 px-3 py-1.5 bg-gray-100 dark:bg-muted border border-gray-200 dark:border-white/5 rounded-full text-sm font-medium text-gray-700 dark:text-gray-200 transition-all hover:border-[#1dff00]/30 hover:bg-[#1dff00]/5"
                    >
                        <span>{item.name}</span>
                        <button
