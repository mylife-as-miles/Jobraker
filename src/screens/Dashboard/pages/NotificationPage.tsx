              ...
              return {
                ...
                icon: (
                  <span className='text-foreground font-bold text-sm'>
                    {(company || "N").charAt(0).toUpperCase()}
                  </span>
                ),
              };
            case "application":
  ...
  useRegisterCoachMarks({
    ...
    marks: [
      ...
      },
      {
        ...
        title: "Filter & Focus",
        body: "Toggle unread, starred or by type; refine further by type & auto-seen preference.",
      },
      {
        id: "notifications-list",
        selector: "#notifications-list",
        title: "Your Inbox",
        body: "Each card is an update. Click one to continue.",
        condition: {
          type: "click",
      ...
      },
      {
        ...
        title: "Detailed View",
        body: "Read the full message, mark it read, archive or delete, or open links.",
      },
      {
        id: "notifications-tour-complete",
        selector: "#notifications-search",
        title: "All Set",
        body: "You're ready to manage updates. Reopen this tour anytime from the floating Tours button.",
      },
    ],
  });
        ...
        <div className='flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6 sm:mb-8'>
          ...
          <div className='flex gap-2 sm:gap-3'>
            ...
            <Button
              ...
              className='product-outline-button text-brand hover:border-brand/50 hover:bg-brand/10 hover:text-brand hover:scale-105 transition-all duration-300 disabled:cursor-not-allowed disabled:opacity-50'
            >
              ...
              >
                {[
                  ...
                  <Button
                    ...
                    className={`${
                      filter === filterOption.key
                        ? "product-control-button-active hover:bg-brand/15"
                  ...
                  onChange={(e) => {
                    ...
                    try {
                      window.dispatchEvent(
                        new CustomEvent("tour:event", {
                          detail: {
                            type: "notifications_type_filter",
                    ...
                    onChange={(e) => {
                      ...
                      try {
                        localStorage.setItem(
                          "notifications:autoMarkSeen",
                          v ? "true" : "false",
                        );
                        window.dispatchEvent(
                          new CustomEvent("tour:event", {
                            detail: {
                              type: "notifications_auto_seen_toggle",
                              value: v,
                            },
                          }),
                        );
              ...
              {filteredNotifications.length === 0 && !loading && (
                <div className='p-8 flex items-center justify-center'>
                  <div className='text-center'>
                    <div className='mx-auto w-14 h-14 rounded-full bg-brand/10 flex items-center justify-center mb-3'>
                    ...
                    <p className='product-helper-text text-xs'>
                      You'll see updates from your job search here.
                    </p>
                  </div>
                </div>
              )}
                  ...
                  <div className='flex items-start space-x-3'>
                    ...
                    <div className='flex-1 min-w-0'>
                      <div className='flex items-start justify-between mb-1'>
                        <p
                          className={`text-sm leading-relaxed font-medium mb-1 ${notification.isRead ? "product-helper-text" : "text-foreground"}`}
                        >
                          {notification.title}
                        </p>
                        <div className='flex items-center space-x-1 flex-shrink-0 ml-2'>
                          <Button
                            ...
                            title={
                              supportsStar
                                ? ""
                                : "Starring requires a DB migration. Please update."
                            }
                            className={`product-helper-text hover:text-brand hover:scale-110 transition-all duration-300 p-1 ${!supportsStar ? "opacity-50 cursor-not-allowed" : ""}`}
                            onClick={(e) => {
                              e.stopPropagation();
                              if (supportsStar) {
                                ...
                                try {
                                  window.dispatchEvent(
                                    new CustomEvent("tour:event", {
                                      detail: {
                                        type: "notification_star_toggle",
                                        id: notification.id,
                                        active: !notification.isStarred,
                                      },
                                    }),
                                  );
                                } catch {}
                          ...
                          >
                            <Star
                              className={`w-3 h-3 ${notification.isStarred ? "fill-current text-brand" : ""}`}
                            />
                          </Button>
                          <Button
                            variant='ghost'
                      ...
                      <div className='flex items-center gap-2'>
                        <p className='text-xs product-helper-text flex items-center gap-1'>
                          ...
                          {!notification.seen_at && (
                            <span className='inline-flex items-center gap-1 px-1.5 py-0.5 rounded bg-brand/15 text-brand text-[10px] font-semibold tracking-wide animate-pulse'>
                              New
                            </span>
                          )}
                        </p>
                        {notification.priority && (
                          <span
                            className={`text-[10px] uppercase tracking-wide px-1.5 py-0.5 rounded bg-muted/50 border ${notification.priority === "high" ? "border-brand text-brand" : notification.priority === "medium" ? "border-brand text-brand" : "border-slate-500 text-slate-400"}`}
                          >
                            {notification.priority}
                          </span>
                        )}
              ...
              {hasMore && (
                <div className='p-3'>
                  <Button
                    variant='ghost'
                    onClick={() => loadMore()}
                    className='w-full text-brand hover:bg-brand/10'
                  >
                    Load more
                  </Button>
                </div>
                  ...
                  <div className='flex items-start space-x-4'>
                    ...
                    <div className='flex-1'>
                      ...
                      {selectedNotificationData.priority && (
                        <p className='mt-1 text-xs product-helper-text'>
                          Priority:{" "}
                          <span
                            className={`font-semibold ${selectedNotificationData.priority === "high" ? "text-brand" : selectedNotificationData.priority === "medium" ? "text-brand" : "text-slate-400"}`}
                          >
                            {selectedNotificationData.priority}
                          </span>
                        </p>
                    ...
                    <div className='flex items-center space-x-2'>
                      <Button
                        ...
                        onClick={() =>
                          ...
                          markRead(selectedNotification, true)
                        }
                      >
                      ...
                      <Button
                        variant='ghost'
                        size='sm'
                        className='product-helper-text hover:text-brand hover:scale-110 transition-all duration-300'
                        onClick={() => {
                ...
                <div className='flex-1 overflow-y-auto p-6'>
                  {selectedNotificationData.hasDetailedContent ? (
                    <div className='space-y-6'>
                      <motion.p
                        className='product-helper-text leading-relaxed'
                        initial={{ opacity: 0, y: 20 }}
                        animate={{ opacity: 1, y: 0 }}
