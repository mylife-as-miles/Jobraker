...
export const SettingsPage = (): JSX.Element => {
  ...
  const renderTabContent = () => {
    switch (activeTab) {
      ...
      case "resume-checker": {
        // Resume Checker inline implementation
        const ResumeCheckerTab = () => {
          ...
          const { apiKey, model: rcModel, baseURL: rcBaseURL } = useOpenAiStore((state) => ({
            apiKey: state.apiKey,
            model: state.model,
          ...
          }, [resumes.length]); // eslint-disable-line react-hooks/exhaustive-deps
          ...
          useEffect(() => {
            ...
            (async () => {
              try {
                ...
                if (!url || aborted) { if (!aborted) lastPreviewedIdRef.current = null; return; }
                const res = await fetch(url);
                if (!res.ok || aborted) { if (!aborted) lastPreviewedIdRef.current = null; return; }
                const blob = await res.blob();
                const text = await (async (blob: Blob, extension: string | null, basename: string): Promise<string> => {
          ...
          const profileSummary = useMemo(() => {
            ...
            const skillLine = skillsData.slice(0, 12).map((skill: any) => skill.level ? `${skill.name} (${skill.level})` : skill.name).join(", ");
            const eduLine = educationData.slice(0, 2).map((ed: any) => `${ed.degree || ""} - ${ed.school || ""}`).join(" | ");
            return [
              `Name: ${fullName}`, `Target Title: ${title}`, `Experience: ${years}`, `Location: ${location}`, `Goals: ${goals}`,
              roleLines.length ? `Recent Roles: ${roleLines.join(" | ")}` : null,
              skillLine ? `Key Skills: ${skillLine}` : null,
              eduLine ? `Education: ${eduLine}` : null,
            ].filter(Boolean).join("\n");
          }, [rcProfile?.id, rcProfile?.first_name, rcProfile?.last_name, rcProfile?.job_title, JSON.stringify(experiencesData.map((e: any) => e.id)), JSON.stringify(educationData.map((e: any) => e.id)), JSON.stringify(skillsData.map((s: any) => s.id))]);
        
          const handleAnalyze = async () => {
            if (!apiKey) { setRcError("Add an OpenAI API key in Settings → Integrations to run the checker."); return; }
            if (!activeResume) { setRcError("Select a resume to analyze."); return; }
            if (!activeResume.file_path) { setRcError("This resume has no file attached yet. Upload or export it first."); return; }
            setRcError(null);
            setRcLoading(true);
            try {
              ...
              const text = await (async (blob: Blob, extension: string | null, basename: string): Promise<string> => {
                ...
                throw new Error("Only PDF or text-based resumes are supported for AI analysis right now.");
              })(blob, activeResume.file_ext, activeResume.name);
                ...
                <div className="grid gap-6 lg:grid-cols-[300px_1fr]">
                  <div className="space-y-4">
                    <div className="rounded-2xl border border-white/10 bg-gradient-to-br from-[#0d0d0d] via-[#111] to-[#0a0a0a] p-4">
                      <div className="space-y-3">
                        {!resumes.length ? (
                          <p className="text-sm text-white/60">No resumes found. Create or import one from the Resume Builder.</p>
                        ) : (
                        ...
                        <Button onClick={handleAnalyze} disabled={rcLoading || !selectedResume || !apiKey} className="w-full bg-[#1dff00]/80 text-black hover:bg-[#1dff00]">
                  ...
                  <div className="space-y-5">
                      ...
                      <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
                        <div>
                          ...
                          <div className="mt-2 flex items-end gap-3">
                            <span className="text-5xl font-black text-white">{analysis ? analysis.overallScore : "--"}</span>
                        ...
                        <div className="flex flex-col items-center justify-center rounded-2xl border border-[#1dff00]/40 bg-black/60 px-6 py-4 text-white">
                          ...
                          <span className="mt-2 text-4xl font-black text-[#1dff00]">{analysis ? analysis.grade : "–"}</span>
                    ...
                    <div className="grid gap-4 lg:grid-cols-2">
                      {[
                        { title: "Key Strengths", icon: ShieldCheck, items: analysis?.strengths, placeholder: "Run the checker to surface signature wins worth keeping.", accent: "positive" as const },
                        { title: "Critical Gaps", icon: AlertTriangle, items: analysis?.gaps, placeholder: "Gaps and ATS blockers will appear here.", accent: "warning" as const }
