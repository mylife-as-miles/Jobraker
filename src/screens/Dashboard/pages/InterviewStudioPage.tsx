...
export const InterviewStudioPage: React.FC = () => {
  ...
  useRegisterCoachMarks({
    ...
    marks: [
      { id: 'interview-viewfinder', selector: '[data-tour="interview-viewfinder"]', title: 'Camera Preview', body: 'Position yourself in frame.' },
      { id: 'interview-record-btn', selector: '[data-tour="interview-record-btn"]', title: 'Start Interview', body: 'Connect with your AI Interviewer.' },
      { id: 'interview-script', selector: '[data-tour="interview-script"]', title: 'Studio Script', body: 'Edit and read your pitch here.' },
      { id: 'interview-metrics', selector: '[data-tour="interview-metrics"]', title: 'Live Coaching', body: 'Monitor your pacing and clarity.' }
    ]
  });
  ...
  const enumerateDevices = useCallback(async () => {
    try {
      ...
      const cameras = devices.filter(d => d.kind === 'videoinput').map(d => ({ deviceId: d.deviceId, label: d.label || `Camera ${d.deviceId.slice(0, 4)}` }));
  ...
  useEffect(() => {
    if (streamInitialized && (selectedCamera || selectedMic)) initializeStream();
  }, [selectedCamera, selectedMic]);
  ...
  return (
    <div className="h-full bg-black text-white overflow-hidden flex flex-col relative">
      ...
      <div className="flex-1 flex flex-col xl:grid xl:grid-cols-12 gap-4 p-4 min-h-0 z-10 overflow-y-auto xl:overflow-hidden">
        ...
        <div className="xl:col-span-12 flex items-center justify-between shrink-0 xl:mb-2">
          <div className="flex items-center gap-2">
            ...
            <h1 className="font-bold font-mono tracking-tight text-lg">Interview Studio</h1>
          </div>
          {isRecording && (
        ...
        <div className="xl:col-span-8 flex flex-col gap-4 min-h-[50vh] xl:min-h-0">
          {/* Viewfinder */}
          <div className="relative flex-1 bg-black rounded-xl border border-white/10 overflow-hidden shadow-2xl group min-h-0" data-tour="interview-viewfinder">

            {/* Cornerstone Markers */}
            <div className="absolute top-4 left-4 w-4 h-4 border-l-2 border-t-2 border-white/20 rounded-tl-sm z-20" />
            <div className="absolute top-4 right-4 w-4 h-4 border-r-2 border-t-2 border-white/20 rounded-tr-sm z-20" />
            <div className="absolute bottom-4 left-4 w-4 h-4 border-l-2 border-b-2 border-white/20 rounded-bl-sm z-20" />
            <div className="absolute bottom-4 right-4 w-4 h-4 border-r-2 border-b-2 border-white/20 rounded-br-sm z-20" />

            {/* Video Container */}
            <div className="w-full h-full flex items-center justify-center bg-[#050505]">
              <div className="relative w-full h-full xl:max-w-[90%] xl:max-h-[95%] aspect-video bg-zinc-900 rounded-lg overflow-hidden ring-1 ring-white/5">
                <video ref={videoRef} autoPlay muted playsInline className={`w-full h-full object-cover transform ${!cameraEnabled ? 'opacity-0' : 'opacity-100'} transition-opacity duration-300`} />
                ...
                {(!cameraEnabled || !streamInitialized) && (
                  <div className="absolute inset-0 flex flex-col items-center justify-center text-zinc-600 bg-zinc-900">
                    {cameraEnabled ? <Camera className="w-10 h-10 mb-2 opacity-50 animate-pulse" /> : <Video className="w-10 h-10 mb-2 opacity-30" />}
                    <p className="text-xs font-mono">{cameraEnabled ? 'INITIALIZING...' : 'CAMERA OFF'}</p>
                  </div>
                )}
                ...
                <AnimatePresence>
                  {isRecording && prompt && (
                    <motion.div initial={{ y: 20, opacity: 0 }} animate={{ y: 0, opacity: 1 }} exit={{ opacity: 0 }} className="absolute bottom-0 left-0 right-0 p-6 bg-gradient-to-t from-black via-black/80 to-transparent pt-12 text-center z-30">
                      <h3 className="text-lg md:text-xl font-medium text-white drop-shadow-lg leading-relaxed">"{prompt}"</h3>
                    </motion.div>
                  )}
                </AnimatePresence>
              </div>
          ...
          {/* Quick Actions */}
          <div className="h-16 bg-zinc-900/40 backdrop-blur-md rounded-xl border border-white/10 flex items-center justify-between px-4 sm:px-6 shrink-0">
            <div className="flex items-center gap-2">
              <AudioVisualizer isActive={isRecording && micEnabled} />
              <div className="h-6 w-[1px] bg-white/10 mx-2" />
              <Button variant="ghost" size="icon" onClick={toggleMic} disabled={isRecording} className={`h-9 w-9 rounded-full ${!micEnabled ? 'bg-red-500/10 text-red-500' : 'text-zinc-400 hover:text-white hover:bg-white/5'}`}>
                {micEnabled ? <Mic size={18} /> : <MicOff size={18} />}
              </Button>
              <Button variant="ghost" size="icon" onClick={toggleCamera} disabled={isRecording} className={`h-9 w-9 rounded-full ${!cameraEnabled ? 'bg-red-500/10 text-red-500' : 'text-zinc-400 hover:text-white hover:bg-white/5'}`}>
              ...
              <Dialog>
                <DialogTrigger asChild>
                  <Button variant="ghost" size="icon" disabled={isRecording} className="h-9 w-9 rounded-full text-zinc-400 hover:text-white hover:bg-white/5">
                    <Settings2 size={18} />
                  </Button>
                </DialogTrigger>
        ...
        <div className="xl:col-span-4 flex flex-col gap-4 min-h-0">
          ...
          {/* 1. SCRIPT CARD */}
          <Card className="flex-[1.5] bg-zinc-900/60 backdrop-blur-sm border border-white/5 flex flex-col overflow-hidden group">
            <div className="p-3 border-b border-white/5 flex items-center justify-between bg-white/5">
              ...
              <Button
                ...
                className={`h-6 text-[10px] px-2 uppercase tracking-wider ${isScriptEditing ? 'text-[#1dff00] bg-[#1dff00]/10 hover:bg-[#1dff00]/20' : 'text-zinc-400 hover:text-white'}`}
              >
                {isScriptEditing ? <span className="flex items-center gap-1"><Check size={10} /> Done</span> : 'Edit'}
              </Button>
            </div>
            <div className="flex-1 p-0 relative" data-tour="interview-script">
              <Textarea
                ...
                className={`w-full h-full bg-transparent border-none resize-none focus:ring-0 p-4 font-mono text-sm leading-relaxed text-zinc-300 ${!isScriptEditing && 'cursor-default select-none'}`}
              />
              {!isScriptEditing && isRecording && (
                <div className="absolute inset-x-0 bottom-0 h-16 bg-gradient-to-t from-zinc-900/90 to-transparent pointer-events-none flex items-end justify-center pb-2">
                  <span className="text-[10px] text-[#1dff00] animate-pulse">Auto-scrolling...</span>
                </div>
              )}
            </div>
          </Card>
          ...
          {/* 2. AI COACH */}
          <Card className="flex-1 bg-zinc-900/60 backdrop-blur-sm border border-white/5 flex flex-col p-4 relative overflow-hidden">
            <div className="flex items-center justify-between mb-4">
              <h3 className="text-xs font-bold text-zinc-400 uppercase tracking-wider flex items-center gap-2">
                <Activity size={12} /> AI Coach
              </h3>
              {isRecording && <span className="px-1.5 py-0.5 rounded bg-[#1dff00] text-black text-[9px] font-bold animate-pulse">LIVE</span>}
            </div>

            <div className="grid grid-cols-2 gap-3">
              {/* WPM */}
              <div className="bg-black/40 rounded-lg p-3 border border-white/5 relative group">
                <span className="text-[10px] text-zinc-500 uppercase">Pace</span>
                <div className="flex items-baseline gap-1 mt-1">
                  <span className={`text-2xl font-bold ${wpm > 160 ? 'text-orange-400' : 'text-white'}`}>{wpm}</span>
              ...
              <div className="bg-black/40 rounded-lg p-3 border border-white/5 relative">
                <span className="text-[10px] text-zinc-500 uppercase">Filler Words</span>
                <div className="flex items-baseline gap-1 mt-1">
                  <span className={`text-2xl font-bold ${fillerWordCount > 5 ? 'text-yellow-400' : 'text-white'}`}>{fillerWordCount}</span>
                  <span className="text-[10px] text-zinc-500">detected</span>
                </div>
                <div className="h-1 w-full bg-zinc-800 rounded-full mt-2 overflow-hidden">
                  <motion.div
                    className="h-full bg-yellow-500"
                    animate={{ width: `${Math.min(100, (fillerWordCount / 10) * 100)}%` }}
                  />
                </div>
              </div>
            </div>
            ...
            <div className="mt-auto pt-4 flex items-center justify-center gap-2 opacity-40">
              <Sparkles size={10} />
              <span className="text-[9px] uppercase tracking-widest">Web Speech API</span>
            </div>
          </Card>

          {/* 3. METRICS */}
          <Card className="flex-1 bg-zinc-900/60 backdrop-blur-sm border border-white/5 flex flex-col p-4">
            <h3 className="text-xs font-bold text-zinc-400 uppercase tracking-wider mb-2">Session Metrics</h3>
      ...
      <AnimatePresence>
        {permissionError && (
          <motion.div initial={{ y: 50, opacity: 0 }} animate={{ y: 0, opacity: 1 }} exit={{ y: 50, opacity: 0 }} className="absolute bottom-6 left-1/2 -translate-x-1/2 px-4 py-3 bg-red-900/90 text-white rounded-lg shadow-xl flex items-center gap-3 border border-red-500/50 z-50">
            <AlertCircle size={18} />
            <span className="text-sm">{permissionError}</span>
            <Button size="sm" variant="ghost" onClick={initializeStream} className="h-6 text-xs hover:bg-white/10">Retry</Button>
          </motion.div>
        )}
      </AnimatePresence>
    </div>
