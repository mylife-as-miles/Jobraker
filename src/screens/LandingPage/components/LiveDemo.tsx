...
export const LiveDemo = () => {
  ...
  return (
    <div className="w-full max-w-lg mx-auto font-mono text-xs sm:text-sm">
      <div className="rounded-lg overflow-hidden border border-[#ffd700]/30 bg-background/80 backdrop-blur-xl shadow-[0_0_30px_rgba(255,215,0,0.15)]">
        {/* Terminal Header */}
        <div className="flex items-center justify-between px-4 py-2 bg-[#ffd700]/10 border-b border-[#ffd700]/20">
          <div className="flex items-center space-x-2">
            <div className="w-3 h-3 rounded-full bg-red-500/80"></div>
            <div className="w-3 h-3 rounded-full bg-yellow-500/80"></div>
            <div className="w-3 h-3 rounded-full bg-amber-500/80"></div>
          </div>
          <div className="flex items-center text-[#ffd700]/60 space-x-1">
        ...
        <div
          ref={scrollRef}
          className="h-[300px] p-4 overflow-y-auto space-y-2 scrollbar-thin scrollbar-thumb-[#ffd700]/20 scrollbar-track-transparent"
        >
          <AnimatePresence initial={false}>
            {logs.slice(0, currentLogIndex + 1).map((log, i) => (
              ...
              >
                <span className="text-[#ffd700]/40 shrink-0">[{new Date().toLocaleTimeString()}]</span>
                <div className="flex items-center space-x-2">
                  {log.type === 'success' && <Check className="w-3 h-3 text-[#ffd700]" />}
                  {log.type === 'process' && <div className="w-3 h-3 border-2 border-[#ffd700] border-t-transparent rounded-full animate-spin" />}
                  {log.icon}
                  <span className={`${
                    log.type === 'success' ? 'text-[#ffd700]' :
          ...
          <div className="flex items-center space-x-2 mt-2">
             <span className="text-[#ffd700] animate-pulse">_</span>
          </div>
        </div>

        {/* Status Bar */}
        <div className="px-4 py-2 bg-[#ffd700]/5 border-t border-[#ffd700]/20 flex justify-between text-[#ffd700]/60 text-[10px] uppercase tracking-wider">
          <span>CPU: 12%</span>
          <span>MEM: 432MB</span>
