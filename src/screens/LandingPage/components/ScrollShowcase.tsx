...
const ShowcaseStep = ({ step, index, total }: { step: typeof steps[0], index: number, total: number }) => {
  return (
    <div className="min-h-screen sticky top-0 flex items-center justify-center overflow-hidden border-t border-[#2dd4bf]/10 bg-background">
      <div className="container mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        <div className="grid lg:grid-cols-2 gap-12 items-center">
          ...
          >
            <div className="text-[#2dd4bf] text-6xl md:text-8xl font-mono font-bold opacity-20 mb-4">
              {step.num}
            </div>
          ...
          <div className="order-1 lg:order-2 relative">
            <motion.div
              ...
              transition={{ duration: 0.8 }}
              className="relative aspect-video rounded-xl overflow-hidden border border-[#2dd4bf]/30 shadow-[0_0_50px_rgba(45,212,191,0.1)]"
            >
              <div className="absolute inset-0 bg-[#2dd4bf]/10 mix-blend-overlay z-10" />
              <img
                src={step.image}
                alt={step.title}
              ...
              {/* Overlay UI Mockup */}
              <div className="absolute bottom-4 left-4 right-4 bg-background/80 backdrop-blur-md border border-[#2dd4bf]/20 p-4 rounded z-20">
                <div className="flex items-center gap-3">
                  <div className="w-2 h-2 bg-[#2dd4bf] rounded-full animate-pulse" />
                  <div className="h-2 w-2/3 bg-[#2dd4bf]/20 rounded" />
                </div>
                <div className="mt-2 h-2 w-1/2 bg-[#2dd4bf]/10 rounded" />
              </div>
            </motion.div>
          </div>
        </div>
