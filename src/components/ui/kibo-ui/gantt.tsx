...
export const Gantt: React.FC<GanttProps> = ({
  ...
  return (
    <div ref={scrollRef} className={"relative w-full overflow-auto rounded-2xl border border-[#ffd700]/20 bg-gradient-to-br from-background via-background to-background backdrop-blur-xl shadow-[0_0_30px_rgba(255,215,0,0.15)] " + className} style={{ WebkitOverflowScrolling: 'touch' }}>
      {/* Ambient glow effect */}
      <div className="pointer-events-none absolute -top-20 right-0 h-64 w-64 rounded-full bg-[#ffd700]/10 blur-3xl opacity-40" />
      
      {/* Header timeline scale + zoom controls */}
      <div className="sticky top-0 z-20 border-b border-[#ffd700]/20 bg-background/95 backdrop-blur-xl">
        <div className="flex items-stretch">
          <div className="w-64 flex items-center justify-between gap-3 px-4 py-3 border-r border-[#ffd700]/20">
            <div className="flex items-center gap-2">
              <div className="h-2 w-2 rounded-full bg-[#ffd700] shadow-[0_0_8px_rgba(255,215,0,0.6)]" />
              <span className="text-xs font-medium bg-gradient-to-r from-[#fbbf24] to-[#6dffb0] bg-clip-text text-transparent">Timeline View</span>
            </div>
            <div className="flex items-center gap-1.5 rounded-lg bg-neutral-900/60 p-1 border border-neutral-700/50">
              <button 
                aria-label="Zoom out" 
                className="h-6 w-6 rounded-md bg-transparent hover:bg-neutral-800 text-foreground/60 hover:text-[#ffd700] text-xs font-medium transition-all duration-200 hover:scale-105" 
                ...
                {gridDays.map((d,i) => {
                  ...
                  return (
                    <div 
                      ...
                      className={`py-2.5 text-center border-r border-[#ffd700]/5 last:border-r-0 transition-colors ${isWeekend ? 'bg-white/[0.02]' : ''}`}
                    >
      ...
      <div className="relative" style={{ width: (min && max ? timelineWidth : 0) + 64 }}>
        <div className="absolute inset-y-0 left-64 w-[1px] bg-gradient-to-b from-[#fbbf24]/20 via-[#ffd700]/10 to-transparent" />
        ...
        <div className="absolute inset-y-0 left-64 right-0 pointer-events-none select-none">
            ...
            {gridDays.map((d,i) => {
              ...
              return (
                <div 
                  key={i} 
                  style={{ width: autoDayWidth }} 
                  className={`h-full border-r border-[#ffd700]/[0.03] last:border-r-0 ${isWeekend ? 'bg-white/[0.01]' : ''}`} 
                />
              );
            })}
          </div>
          {showTodayMarker && (
            <div className="absolute top-0 bottom-0" style={{ left: `${todayPercent!*100}%` }}>
              <div className="h-full w-[2px] bg-gradient-to-b from-[#fbbf24] via-[#ffd700]/80 to-[#b45309]/20 shadow-[0_0_8px_rgba(255,215,0,0.5)]" />
              <div className="absolute -top-1 left-1/2 -translate-x-1/2 px-2 py-1 rounded-md bg-[#ffd700] text-black text-[10px] font-bold tracking-wide shadow-lg">
                Today
              </div>
            </div>
          )}
        ...
        <div className="relative">
          {valid.length === 0 && (
            <div className="p-8 text-center">
              <div className="inline-flex items-center justify-center h-16 w-16 rounded-2xl bg-foreground/5 border border-foreground/10 mb-3">
                <div className="h-8 w-8 rounded-lg bg-gradient-to-br from-[#fbbf24]/20 to-transparent" />
              </div>
          ...
          {valid.length > 0 && groups.map(g => (
            <div key={g.key} className="relative">
              {groups.length > 1 && (
                <div className="sticky left-0 z-10 w-64 bg-gradient-to-r from-background to-background/80 backdrop-blur-sm border-r border-[#ffd700]/20 py-2 px-4">
                  <div className="flex items-center gap-2">
                    <div className="h-1.5 w-1.5 rounded-full bg-[#ffd700]/60" />
              ...
              <div className={groups.length>1 ? 'pl-64' : ''}>
                <div className="relative divide-y divide-[#ffd700]/5">
                  {g.rows.map(item => {
                      ...
                      <div key={item.id} className="relative group/item hover:bg-white/[0.02] transition-colors" style={{ height }}>
                        <div className="absolute inset-y-0 left-0 flex items-center pl-4 pr-3 w-64 overflow-hidden">
                          <div className="flex items-center gap-2 w-full">
                            <div className={`h-1.5 w-1.5 rounded-full flex-shrink-0 ${item.status === 'Applied' ? 'bg-[#ffd700]' : item.status === 'Interview' ? 'bg-amber-400' : item.status === 'Offer' ? 'bg-lime-400' : item.status === 'Rejected' ? 'bg-rose-400' : 'bg-gray-400'} shadow-[0_0_4px_currentColor]`} />
                        ...
                        <div className="absolute inset-y-0 left-64" style={{ right: 0 }}>
                          <div className="relative h-full">
                            ...
                            <div
                              className="absolute group rounded-lg overflow-hidden ring-1 ring-white/10 shadow-md hover:ring-[#ffd700]/40 hover:shadow-[0_0_20px_rgba(255,215,0,0.2)] transition-all duration-300 cursor-pointer focus:outline-none focus:ring-2 focus:ring-[#ffd700]/60 transform hover:scale-[1.02]"
                            ...
                            >
                              ...
                              {/* Enhanced Tooltip */}
                              <div className="absolute z-30 hidden group-hover:flex -top-3 left-1/2 -translate-y-full -translate-x-1/2 min-w-[220px] max-w-[280px] flex-col rounded-xl border border-[#ffd700]/30 bg-gradient-to-br from-background to-background backdrop-blur-xl p-3 shadow-[0_0_30px_rgba(255,215,0,0.2)]">
                                <div className="flex items-center gap-2 mb-2 pb-2 border-b border-foreground/10">
                                  <div className={`h-2 w-2 rounded-full ${item.status === 'Applied' ? 'bg-[#ffd700]' : item.status === 'Interview' ? 'bg-amber-400' : item.status === 'Offer' ? 'bg-lime-400' : item.status === 'Rejected' ? 'bg-rose-400' : 'bg-gray-400'}`} />
                                ...
                                <div className="grid grid-cols-2 gap-x-3 gap-y-1.5 text-[11px]">
                                  ...
                                  <span className="text-[#ffd700] font-semibold">{days} day{days !== 1 ? 's' : ''}</span>
                                  {trailEnd && (
                                    <>
...
export const GanttFeatureItem: React.FC<GanttFeatureItemProps> = ({
  ...
  return (
    <div className={"relative h-8 " + className} {...rest}>
      <div
        ref={barRef}
        className="group absolute top-1/2 -translate-y-1/2 rounded-md ring-1 ring-white/10 hover:ring-white/30 hover:shadow-lg shadow-sm cursor-grab active:cursor-grabbing focus:outline-none focus:ring-2 focus:ring-[#ffd700]/60"
...
export const GanttToday: React.FC = () => {
  ...
  return (
    <div className="absolute inset-y-0 pointer-events-none" style={{ left: p + '%' }}>
      <div className="h-full w-px bg-[#ffd700] shadow-[0_0_0_1px_rgba(255,215,0,0.4)]" />
      <div className="absolute -top-2 -translate-y-full -translate-x-1/2 px-1 py-0.5 rounded bg-[#ffd700] text-black text-[9px] font-semibold">Today</div>
    </div>
  );
};
