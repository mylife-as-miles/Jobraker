...
// Minimal internal table primitives (avoid relying on alias '@/components/ui/table')
const TableRaw: React.FC<React.HTMLAttributes<HTMLTableElement>> = ({ className = '', ...rest }) => (
  <div className="relative w-full overflow-auto"><table className={cn("w-full caption-bottom text-sm", className)} {...rest} /></div>
);
