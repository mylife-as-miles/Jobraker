...
export default function SortDropdown({ value, onChange }: SortDropdownProps) {
  ...
  return (
    <div className="relative inline-block text-sm" ref={rootRef}>
      ...
      {isOpen && (
        <div className="absolute z-50 top-full mt-1 w-[180px] bg-background border border-foreground/10 rounded-md shadow-lg">
          <div role="listbox" aria-label="Sort options" className="p-1">
            {options.map((opt) => (
              <button
                ...
                className="w-full flex items-center justify-between px-3 py-2 rounded text-left text-sm text-muted-foreground bg-background"
              >
