...
export const UpgradePrompt: React.FC<UpgradePromptProps> = ({
    ...
    return (
      ...
      >
        ...
        <div className="pointer-events-none absolute inset-0">
          <div className="absolute top-0 right-0 h-64 w-64 rounded-full bg-[#ffd700]/5 blur-3xl" />
          <div className="absolute bottom-0 left-0 h-64 w-64 rounded-full bg-purple-500/5 blur-3xl" />
        </div>

        <div className="relative z-10 text-center">
          <div className="mx-auto mb-4 inline-flex h-16 w-16 items-center justify-center rounded-2xl bg-gradient-to-br from-[#fbbf24]/20 to-purple-500/20 border border-foreground/10">
            <LockIcon className="h-8 w-8 text-[#ffd700]" />
          </div>
          ...
          <Link to="/dashboard/billing">
            <Button className="bg-gradient-to-r from-[#fbbf24] to-background hover:from-[#fbbf24]/90 hover:to-background/90 text-black font-semibold">
              Upgrade Now
  ...
  return (
    ...
    >
      {/* Ambient background effects */}
      <div className='pointer-events-none absolute inset-0'>
        <div className='absolute top-0 right-0 h-96 w-96 rounded-full bg-[#ffd700]/5 blur-3xl animate-pulse' />
        <div
        ...
        <div className='mx-auto max-w-4xl'>
          ...
          <div className='text-center mb-12'>
            <motion.div
              ...
              className='mx-auto mb-6 inline-flex h-20 w-20 sm:h-24 sm:w-24 items-center justify-center rounded-3xl bg-gradient-to-br from-[#fbbf24]/20 via-purple-500/10 to-blue-500/10 border border-foreground/10 backdrop-blur-xl'
            >
              {icon || (
                <LockIcon className='h-10 w-10 sm:h-12 sm:w-12 text-[#ffd700]' />
              )}
            </motion.div>
            ...
            >
              {features.map((feature, index) => (
                <motion.div
                  key={index}
                  variants={itemVariants}
                  className='group relative overflow-hidden rounded-2xl border border-foreground/10 bg-gradient-to-br from-foreground/5 to-foreground/[0.02] p-6 backdrop-blur-sm hover:border-[#ffd700]/30 transition-all duration-300'
                >
                  <div className='absolute inset-0 bg-gradient-to-br from-[#fbbf24]/0 to-[#b45309]/0 group-hover:from-[#fbbf24]/5 group-hover:to-transparent transition-all duration-300' />

                  <div className='relative z-10'>
                    {feature.icon && (
                      <div className='mb-3 inline-flex h-10 w-10 items-center justify-center rounded-xl bg-[#ffd700]/10 text-[#ffd700] group-hover:bg-[#ffd700]/20 transition-colors'>
                        {feature.icon}
                      </div>
                    )}
            ...
            >
              {planCards.map((plan) => {
                ...
                const borderClass = isBasics
                  ? "border-[#ffd700]/30 bg-gradient-to-br from-[#fbbf24]/10 via-[#ffd700]/5 to-transparent hover:border-[#ffd700]/50"
                  : isPro
