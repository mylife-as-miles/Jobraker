      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-sm">
        <div>
          ...
          <div className="flex flex-wrap gap-2">
            {matched.map((s, i) => (
              <span key={i} className="px-2 py-1 bg-[#2dd4bf20] text-[#2dd4bf] text-xs rounded border border-[#2dd4bf]/30">
                {s}
              </span>
            ))}
            {!matched.length && <span className="text-foreground/40">None</span>}
          </div>
        </div>
        <div>
          ...
          <div className="flex flex-wrap gap-2">
            {missing.map((s, i) => (
              <span key={i} className="px-2 py-1 bg-foreground/10 text-white text-xs rounded border border-foreground/20">
                {s}
              </span>
            ))}
            {!missing.length && <span className="text-foreground/40">None</span>}
          </div>
        </div>
      </div>
    </Card>
