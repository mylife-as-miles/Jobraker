...
function MetricTile({ label, value, delta, absolute = false }: MetricTileProps) {
      ...
      <div className="flex items-end justify-between gap-2">
        ...
        {!isNeutral && (
          <span
            className={
              isPositive
                ? "inline-flex items-center gap-0.5 rounded-full bg-[#1dff00]/10 px-1.5 py-0.5 text-xs font-bold text-[#1dff00]"
...
export function WeeklyDigestCard({ weeklyDigest, loading }: WeeklyDigestCardProps) {
  return (
    <motion.div
