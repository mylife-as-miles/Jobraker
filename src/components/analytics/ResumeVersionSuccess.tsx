...
export function ResumeVersionSuccess({ period, data }: { period: Period; data: any }) {
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.45, delay: 0.15 }} className="h-full">
        ...
        <div className="flex h-full flex-col p-5 sm:p-6">
          <div className="mb-5 flex items-start justify-between gap-3">
            <div>
              ...
              <h2 className="mt-3 text-2xl font-semibold text-foreground">Application status mix</h2>
              <p className="mt-1 text-sm text-foreground/65">Pipeline breakdown for {String(period).toUpperCase()}</p>
            </div>
            <div className="rounded-2xl border border-border bg-background/70 px-4 py-3 text-right">
              <div className="text-[11px] uppercase tracking-wide text-foreground/50">Total applications</div>
              <div className="mt-1 text-3xl font-bold text-foreground">{totalApplications}</div>
            </div>
          </div>
            ...
            <div className="space-y-3">
              {hasData ? (
                chartData.map((item: any) => (
                  <div key={item.name} className="flex items-center justify-between rounded-2xl border border-border bg-background/60 px-4 py-3">
                    <div className="flex items-center gap-3">
                      <span className="h-3.5 w-3.5 rounded-full" style={{ backgroundColor: item.color }} />
                      <div>
                        <div className="font-medium text-foreground">{item.name}</div>
                        <div className="text-xs text-foreground/55">{item.share ?? 0}% of applications</div>
                      </div>
                    </div>
