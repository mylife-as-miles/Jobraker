...
import { Briefcase, CalendarRange, DollarSign, Sparkles, Timer } from "lucide-react";
...
export function PipelineEarningsProjectionCard({
  ...
  const [sim, setSim] = useState<PipelineSimulationParams>({ ...DEFAULT_PIPELINE_SIM });

  const range = useMemo(() => computeAnalyticsDateRange(period), [period]);

  const rangeLabel = useMemo(() => {
    const a = range.start.toLocaleDateString(undefined, { month: "short", day: "numeric", year: "numeric" });
    ...
    <Card className="rounded-2xl border border-[#ffd700]/25 bg-gradient-to-br from-card via-card to-foreground/[0.02] p-5 sm:p-6 shadow-[0_0_40px_rgba(255,215,0,0.06)]">
      <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
        <div className="min-w-0 space-y-1">
          ...
          <p className="text-sm text-foreground/55">
            Probability-weighted rollups from <strong className="text-foreground/80">Interview</strong> and{" "}
            <strong className="text-foreground/80">Offer</strong> applications with parsed compensation text (annualized,
            nominal USD).
          </p>
        </div>
      ...
      <div className="mt-4 flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
        <div className="flex flex-wrap items-center gap-2">
          ...
          <Button
            ...
            onClick={() => navigate("/dashboard/application?view=list&status=Offer")}
          >
            Open offers
          </Button>
          <Button type="button" variant="ghost" size="sm" className="h-8 text-xs text-foreground/50" onClick={resetSimulation}>
            Reset
          </Button>
        </div>
        ...
        <div className="mt-4 grid gap-4 sm:grid-cols-3">
          <div>
            ...
            <Slider
              ...
              onValueChange={([v]) => setSim((s) => ({ ...s, interviewToOffer: (v ?? 28) / 100 }))}
          ...
          <div>
            ...
            <Slider
              ...
              onValueChange={([v]) => setSim((s) => ({ ...s, offerRealized: (v ?? 55) / 100 }))}
          ...
          <div>
            ...
            <Slider
              ...
              onValueChange={([v]) => setSim((s) => ({ ...s, negotiationUplift: (v ?? 0) / 100 }))}
      ...
      <div className="mt-6 grid gap-4 lg:grid-cols-[1fr_200px] lg:gap-6">
        <div className="min-h-[280px] w-full">
          {loading ? (
            <div className="flex h-[280px] items-center justify-center text-sm text-foreground/50">Loading applications…</div>
          ) : chartData.length < 2 ? (
            <div className="flex h-[280px] flex-col items-center justify-center gap-2 rounded-xl border border-dashed border-foreground/15 bg-foreground/[0.02] px-4 text-center">
              <p className="text-sm font-medium text-foreground/70">No compensable pipeline in this window</p>
              <p className="text-xs text-foreground/45">
                Add compensation on <strong>Offer</strong> or <strong>Interview</strong> rows (Application detail →
        ...
        <div className="flex flex-col justify-center gap-3 rounded-xl border border-foreground/10 bg-foreground/[0.03] p-4 lg:border-l lg:border-t-0 lg:border-foreground/10">
          ...
          <div className="space-y-1 text-xs text-foreground/55">
            ...
            <p className="text-[10px] leading-relaxed text-foreground/40">
              Modeled cumulative comp if listings/offers parse cleanly. Not tax or offer advice—tune sliders to stress-test
              your pipeline.
            </p>
          </div>
        </div>
