...
export function IndustriesCard({ period, data }: { period: Period; data: any }) {
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.45, delay: 0.05 }} className="h-full">
        ...
        <div className="flex h-full flex-col p-5 sm:p-6">
          <div className="mb-5 flex items-start justify-between gap-3">
            <div>
              ...
              <h3 className="mt-3 text-2xl font-semibold text-foreground">Where jobs are coming from</h3>
              <p className="mt-1 text-sm text-foreground/65">Top discovery channels in {String(period).toUpperCase()}</p>
            </div>
            <div className="rounded-2xl border border-border bg-background/70 px-3 py-2 text-right">
              <div className="text-[11px] uppercase tracking-wide text-foreground/50">Distinct sources</div>
              <div className="mt-1 text-2xl font-bold text-foreground">{metrics.sources}</div>
            </div>
          </div>
          ...
          <div className="mb-4 grid grid-cols-2 gap-3">
            <div className="rounded-2xl border border-border bg-background/60 p-3">
              <div className="flex items-center gap-2 text-xs uppercase tracking-wide text-foreground/50">
                <SearchCheck className="h-4 w-4 text-[#2dd4bf]" />
                Jobs found
              </div>
          ...
          <div className="min-h-[260px] flex-1">
            {hasData ? (
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={sourceData} layout="vertical" margin={{ top: 8, right: 8, left: 12, bottom: 0 }}>
