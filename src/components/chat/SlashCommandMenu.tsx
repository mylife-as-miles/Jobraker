    ...
    <AnimatePresence>
      {open && list.length > 0 && (
        <motion.div
          ...
          ref={containerRef}
        >
          <div className="rounded-xl overflow-hidden border border-[#ffd700]/30 bg-background/95 backdrop-blur-xl shadow-xl">
            <ul className="divide-y divide-[#ffd700]/10">
              {list.map(item => (
                <li key={item.cmd}>
                  <button
                    data-slash-item
                  ...
                  >
                    <span className="text-[12px] font-semibold text-[#b9ffb5]"><span className="text-[#ffd700]">/{item.cmd}</span> — {item.title}</span>
