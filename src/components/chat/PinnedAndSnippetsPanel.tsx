    ...
    <AnimatePresence>
      {open && (
        <motion.aside
          ...
          className="fixed top-0 right-0 h-full w-[360px] z-[160] bg-background/95 backdrop-blur-xl border-l border-[#ffd700]/20 flex flex-col"
        >
          <div className="flex items-center justify-between px-4 py-3 border-b border-[#ffd700]/20">
            <div className="flex items-center gap-3 text-sm font-semibold tracking-wide text-[#ffd700]">
                ...
                {pinnedMessages.map(pm => (
                  <div key={pm.id} className="group border border-[#ffd700]/25 rounded-xl p-3 bg-background/60 hover:bg-[#141f14]/70 transition">
                    ...
                    <div className="flex justify-end pt-2">
                      <button onClick={()=>unpinMessage(activeSessionId!, pm.id)} className="text-[10px] px-2 py-1 rounded-md bg-[#ffd700]/10 text-[#ffd700] hover:bg-[#ffd700]/20 transition">Unpin</button>
                    </div>
                  </div>
                ))}
              </div>
          ...
          <div className="px-4 py-2 border-t border-[#ffd700]/20 text-[10px] text-neutral-500 tracking-wider flex justify-between">
