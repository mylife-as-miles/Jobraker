        ...
        >
          <div className="flex items-center justify-between px-4 py-3 border-b border-[#1dff00]/20">
            ...
            <div className="flex items-center gap-2">
              <button onClick={() => setTab('pinned')} className={`text-[11px] px-2 py-1 rounded-md border ${tab==='pinned'?'border-[#1dff00]/60 text-[#1dff00] bg-[#3a2f0c]':'border-[#1dff00]/20 text-neutral-400 hover:text-[#1dff00]'}`}>Pinned</button>
              <button onClick={() => setTab('snippets')} className={`text-[11px] px-2 py-1 rounded-md border ${tab==='snippets'?'border-[#1dff00]/60 text-[#1dff00] bg-[#3a2f0c]':'border-[#1dff00]/20 text-neutral-400 hover:text-[#1dff00]'}`}>Snippets</button>
            ...
            {tab === 'pinned' && (
              <div className="space-y-2">
                ...
                {pinnedMessages.map(pm => (
                  <div key={pm.id} className="group border border-[#1dff00]/25 rounded-xl p-3 bg-background/60 hover:bg-[#2d2610]/70 transition">
            ...
            {tab === 'snippets' && (
              <div className="space-y-2">
                ...
                {snippets.slice().sort((a,b)=>b.createdAt - a.createdAt).map(sn => (
                  <div key={sn.id} className="group border border-[#1dff00]/25 rounded-xl p-3 bg-[#1c170d]/60 hover:bg-[#2a2310]/70 transition">
                    <div className="flex items-start gap-2">
                      <BookmarkPlus className="w-4 h-4 text-[#1dff00] mt-0.5" />
                      <div className="flex-1">
                        <div className="text-[11px] font-semibold text-[#1dff00] mb-1 truncate">{sn.title}</div>
                        <div className="text-[12px] leading-relaxed text-neutral-300 whitespace-pre-wrap">{sn.content.slice(0, 1000)}</div>
                      </div>
                    </div>
                    <div className="flex justify-end pt-2 gap-2">
                      <button onClick={()=>deleteSnippet(sn.id)} className="text-[10px] px-2 py-1 rounded-md bg-[#1dff00]/10 text-[#1dff00] hover:bg-[#1dff00]/20 transition flex items-center gap-1"><Trash2 className="w-3 h-3" />Delete</button>
                    </div>
                  </div>
                ))}
              </div>
