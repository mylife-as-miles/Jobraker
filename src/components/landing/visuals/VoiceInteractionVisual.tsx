    ...
    <div className="w-full h-full min-h-[200px] flex items-center justify-center relative overflow-hidden bg-background">
      ...
      <div className="relative z-10">
        <motion.div
          ...
          className="w-16 h-16 rounded-full bg-[#2dd4bf]/20 blur-md absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2"
        />
        <div className="w-12 h-12 rounded-full border-2 border-[#2dd4bf] flex items-center justify-center relative bg-black z-20 shadow-[0_0_15px_rgba(45,212,191,0.3)]">
      ...
      <div className="flex items-center space-x-1 absolute left-1/2 -translate-x-16 mr-4 h-16">
         {[...Array(5)].map((_, i) => (
            <motion.div
               ...
               className="w-1 bg-[#2dd4bf]/50 rounded-full"
            />
         ))}
      </div>
      ...
      <motion.div
         initial={{ opacity: 0 }}
         animate={{ opacity: 1 }}
         className="absolute bottom-4 text-[10px] font-mono text-[#2dd4bf] uppercase tracking-widest"
      >
         Listening...
      </motion.div>
    </div>
