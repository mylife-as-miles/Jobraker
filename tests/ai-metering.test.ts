...
describe("AI Token Pricing & Nanodollar Integer Accounting", () => {
  ...
  it("calculates mixed input/output token usage accurately", () => {
    // 100,000 input tokens ($0.05 = 50,000,000 nanos) + 50,000 output tokens ($0.15 = 150,000,000 nanos) = $0.20 (200,000,000 nanos)
  ...
  it("handles very large token counts without float overflow or precision loss", () => {
    const hugeInput = 10_000_000_000n; // 10 billion tokens
    const hugeOutput = 5_000_000_000n; // 5 billion tokens
    const cost = calculateCostNanos(hugeInput, hugeOutput);

    // 10B * 500 = 5,000,000,000,000 nanos ($5,000)
    // 5B * 3,000 = 15,000,000,000,000 nanos ($15,000)
    // Total = 20,000,000,000,000 nanos ($20,000)
...
describe("Multi-Window Enforcement Logic", () => {
  it("rejects a request when the weekly limit is exhausted even if monthly capacity remains", () => {
    const pro = getTierLimits("Pro");
    const monthlyUsed = 3_000_000_000n; // $3.00 used out of $12.00 monthly (plenty left)
    const weeklyUsed = 4_800_000_000n;  // $4.80 used out of $4.80 weekly (exhausted!)
    ...
    expect(weeklyStatus.percentLeft).toBe(0); // Rejected by weekly window!
  });
});
