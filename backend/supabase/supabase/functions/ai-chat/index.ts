...
Deno.serve(async (req: Request): Promise<Response> => {
  ...
  const mode = body.model;

  if (mode === 'ask') {
    const schema = await Deno.readTextFile('./_shared/schema/schema.sql');
    const systemPrompt = `You are a helpful AI assistant. You are an expert in the application's database schema. The user is asking you a question about the application. Your role is to answer the user's question based on the database schema. Here is the database schema:\n\n${schema}\n\n`;
    ...
    const tools = [
      {
        ...
        function: {
          name: 'search_jobs',
          description: 'Searches for jobs based on a query, location, and limit.',
          parameters: {
            type: 'object',
            properties: {
              query: {
    ...
    const initialResponse = await fetch(OPENAI_BASE, {
      method: 'POST',
      headers: {
    ...
    const toolCall = initialData.choices[0].message.tool_calls && initialData.choices[0].message.tool_calls[0];
    ...
    if (toolCall) {
      ...
      if (toolName === 'search_jobs') {
        const { query, location, limit } = toolArgs;

        const jobsSearchResponse = await fetch(`${Deno.env.get('SUPABASE_URL')}/functions/v1/jobs-search`, {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
        ...
        const jobsSearchData = await jobsSearchResponse.json();

        providerMessages.push({
          role: 'tool',
          tool_call_id: toolCall.id,
          content: JSON.stringify(jobsSearchData),
        });

        body.messages = providerMessages;
        body.model = 'openai/gpt-4o-mini';
      }
    }
  }

  // Determine provider & model for standard proxy mode
