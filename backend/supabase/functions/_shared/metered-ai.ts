...
// Input: $0.50 / 1M => 500 nanodollars per token.
// Output: $3.00 / 1M => 3,000 nanodollars per token.
...
/** Extract Gemini usage metadata without silently treating estimates as provider facts. */
...
export async function reserveAiUsage(
  ...
  if (!result || result.success !== true) {
    ...
    if (code === "AI_USAGE_LIMIT_REACHED") {
      const window = result?.window === "weekly" || result?.window === "monthly"
        ? result.window
        : "rolling_24h";
...
/** Explicitly named bypass for reviewed background-only operations. */
...
/**
 * Executes exactly one provider call for one newly-created reservation.
 * Provider failures, provider-usage gaps, and ledger failures remain distinct.
 */
