  ...
  return items
    .map((entry) => {
      ...
      return {
        name,
        careers_url:
          asString(raw.careers_url) ||
          asString(raw.careersUrl) ||
          asString(raw.url),
        source_hint:
          asString(raw.source_hint) ||
          asString(raw.sourceHint) ||
          asString(raw.board_type),
...
const deriveStories = (
  ...
  return experiences.slice(0, 4).map((experience) => ({
    title:
      asString(experience.title) ||
      asString(experience.company) ||
      "Career story",
    situation:
...
const formatCandidateMemorySummary = (memory: Omit<CandidateMemory, "summaryText">): string => {
  ...
  if (memory.trackedCompanies.length > 0) {
    lines.push(
      `Tracked companies: ${memory.trackedCompanies
        .slice(0, 10)
...
export async function fetchCandidateMemory(
  ...
  const [profileRes, experiencesRes, skillsRes, parsedResumeRes] =
    await Promise.all([
      serviceClient
        .from("profiles")
        .select(
  ...
  const proofPoints = deriveProofPoints(
    explicitProofPoints,
    experiencesRes.data ?? [],
    parsedResume,
  );
  const explicitStories = normalizeStories(profile.story_bank);
  const storyBank = deriveStories(
    explicitStories,
    experiencesRes.data ?? [],
    proofPoints,
  );
  ...
  const memoryWithoutSummary: Omit<CandidateMemory, "summaryText"> = {
    fullName,
    headline:
      asString(profile.job_title) || asString(parsedResume?.summary) || null,
