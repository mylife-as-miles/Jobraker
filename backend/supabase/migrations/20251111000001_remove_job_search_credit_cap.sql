-- Remove the 10 credit cap from job search credit deduction
-- Now deducts 1 credit per job found (no maximum limit)
...
DECLARE
    ...
    v_result JSON;
BEGIN
    -- Calculate credits to deduct (1 credit per job, NO CAP)
    v_credits_to_deduct := p_jobs_count;
...
COMMENT ON FUNCTION public.deduct_job_search_credits IS 'Deducts credits for job searches using transaction_type=deduction (1 credit per job, no maximum limit)';
